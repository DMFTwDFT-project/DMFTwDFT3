...
import Struct,Fileio,VASP, DMFT_MOD, IMP_SOLVER_HF
from scipy import *
      ...
      for it in range(p['Nit']):
         ...
         if itt==0 and it==0:
            for i,ats in enumerate(cor_at):
               ...
               IMP_SOLVER_HF.Create_atomd(pD)
               IMP_SOLVER_HF.Create_Trans(TB.ncor_orb_list[i],p['nspin'],ats[0],cor_orb[i],TB)
...
#            fi.close()
       ...
       # shutil.copy2('Delta.out','Delta1.inp')
         ...
         if TB.LHF=='.FALSE.': IMP_SOLVER_HF.RUN_CTQMC(p,pC,pD,it,itt,para_com,DMFT.mu,Ed,DMFT.Vdc)
