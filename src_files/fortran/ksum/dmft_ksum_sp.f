...
!                  totnk(ikp)=totnk(ikp)+2*fermi((t_eval0(nb,ispin,ikp)
!     1                  -mu)/T)*sweight
                  do im=1,noms
...
!                    totnk(ikp)=totnk(ikp)+4*T*real(Gs-Gs0)
!     1               *sweight
                  enddo
                enddo
              enddo
            endif
            !write(*,*) kpt_latt(:,nkp),totnk(ikp)
          enddo
          total_n=0
     ...
     1            +dcmplx(SigMdc(2*i,im,ispin),SigMdc(2*i+1,im,ispin))
                ...
                do nb=1,num_wann
                  Gs0=(mu+dcmplx(0.0,SigMdc(1,im,ispin))-eval0(nb))
                  Gs=(mu+dcmplx(0.0,SigMdc(1,im,ispin))-evalue(nb))
     ...
     1        MPI_SUM,MPI_Comm_World,ierr)
        ...
        allocate(old_Sig(2*ncor_orb))
        allocate(new_Sig(2*ncor_orb))
     ...
     1          *dm_ud((i-1)*2*norb+l)
            ...
            enddo
          Ekin=Ekin-SigMdc(2*i,nom,1)*(tot_dm(i)+tot_mom(i))
          if (nspin.eq.2) then
             Ekin=Ekin-SigMdc(2*i,nom,2)*(tot_dm(i)-tot_mom(i))
          else
             Ekin=Ekin-SigMdc(2*i,nom,1)*(tot_dm(i)-tot_mom(i))
          endif
...
!          Epot_st=Epot_st-Up(i)*Nd*(Nd-1.0)/2.0+Jh(i)*Nd*(Nd-2.0)/4.0
        enddo
        !if (rank.eq.0) write(*,*) 'hi'     
!        do i=1,ncor_orb/norb
!          do j=1,norb
!            old_Sig((i-1)*2*norb+j)=SigMdc(2*((i-1)*norb+j),nom,1)
!            if (nspin.eq.2) then
!             old_Sig((i-1)*2*norb+norb+j)=SigMdc(2*((i-1)*norb+j),nom,2)
!            endif
!          enddo
!        enddo
!        !if (rank.eq.0) write(*,*) old_Sig
!        !if (rank.eq.0) write(*,*) Sig
!        new_Sig=old_Sig+mix_Sig*(Sig-old_Sig)
!        !if (rank.eq.0) write(*,*) new_Sig
!        do i=1,2*ncor_orb
...
!          do j=1,norb
!            do im=1,nom
!              SigMdc(2*((i-1)*norb+j),im,1)=SigMdc(2*((i-1)*norb+j),im,1
!     1         )+new_Sig((i-1)*2*norb+j)
!              if (nspin.eq.2) then
!                SigMdc(2*((i-1)*norb+j),im,2)=SigMdc(2*((i-1)*norb+j),im
!     1           ,2)+new_Sig((i-1)*2*norb+norb+j)
!              endif
!            enddo
!          enddo
!        enddo
      ...
      if (rank.eq.0) then
        do ispin=1,nspin
          do i=1,ncor_orb
            ...
            !write(*,*) Sig0,SigMdc(2*i+1,nom,ispin),SigMdc(1,nom,ispin)
            do j=1,nom
              Epot_dy=Epot_dy+4.0*T*real(tot_Gloc(i,j,ispin)
...
!            Epot_dy=Epot_dy+Sig1/(4*T)*sweight
            do j=1,noms
              tot_Ekin=tot_Ekin-4.0*T*real(tot_Gloc(i,j,ispin)
     1       *(dcmplx(SigMdc(2*i,j,ispin)-Sig0,SigMdc(2*i+1,j,ispin))))
     1       *sweight
            enddo
          enddo
...
!        !write(*,*) write_format
!        WRITE(99,write_format) (SigMdc(2*i,nom,1),i=1,ncor_orb)
!        CLOSE(99)
!        if (nspin.eq.2) then
!          OPEN(UNIT=99,FILE='SigMdc_dn.out',FORM='FORMATTED')
! 121      FORMAT('(',I2,'F18.12)')
!          WRITE(write_format,121) ncor_orb
!          !write(*,*) write_format
!          WRITE(99,write_format) (SigMdc(2*i,nom,2),i=1,ncor_orb)
!          CLOSE(99)
!        endif
...
!        CLOSE(99)
 ...
 132    FORMAT('(',I2,'F18.12)')
     ...
     1SUM(tot_dm(ncor_orb-norb+1:ncor_orb)), tot_Ekin+mu*total_n, 
     1 Epot_st+Epot_dy,SigMdc(2,nom,1), SigMdc(2*ncor_orb,nom,1)  
        CLOSE(99)
      endif
